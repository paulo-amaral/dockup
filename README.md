...
[![ci](https://github.com/paulo-amaral/Easy-Install-docker-ce-docker-compose/actions/workflows/ci.yml/badge.svg)](https://github.com/paulo-amaral/Easy-Install-docker-ce-docker-compose/actions/workflows/ci.yml)
[![release](https://img.shields.io/github/v/release/paulo-amaral/Easy-Install-docker-ce-docker-compose)](https://github.com/paulo-amaral/Easy-Install-docker-ce-docker-compose/releases/latest)
...
The bootstrap downloads the latest release binary for your OS/arch, **verifies its SHA-256 checksum** against the release manifest and launches the TUI. Or grab a binary from [Releases](https://github.com/paulo-amaral/Easy-Install-docker-ce-docker-compose/releases) and run `dockup`.
...
| `audit` | Read-only checks inspired by the CIS Docker Benchmark: socket permissions, `docker` group members, daemon flags, privileged containers |
| `status` / `prune` | Engine health, `docker system df`, cleanup of unused data (with confirmation) |
| `apple-install` / `apple-start` / `apple-status` | [Apple's open source `container`](https://github.com/apple/container) on macOS Apple Silicon, via Homebrew |
...
```sh
git clone https://github.com/paulo-amaral/Easy-Install-docker-ce-docker-compose.git
cd Easy-Install-docker-ce-docker-compose
...
*Keywords: docker installer, docker compose v2 install, docker tui, terminal ui, nvidia container toolkit installer, gpu docker, docker security hardening, CIS docker benchmark, apple container macos, homelab, devops tools, bubbletea*
